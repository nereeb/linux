					 ...
					 struct gates_data *data)
{
	...
	/* Worst-case approximation and memory allocation */
	qty = find_last_bit(data->mask, 64);
	clk_data = kmalloc(sizeof(struct clk_onecell_data), GFP_KERNEL);
	clk_data->clks = kzalloc((qty+1) * sizeof(struct clk *), GFP_KERNEL);

	for_each_set_bit(i, data->mask, sizeof(data->mask)) {
		of_property_read_string_index(node, "clock-output-names",
					      j, &clk_name);

		clk_data->clks[j] = clk_register_gate(NULL, clk_name, clk_parent, CLK_IGNORE_UNUSED,/*HACK*/
						      reg + 4 * (i/32), i % 32,
						      0, &clk_lock);
